...
DOWNLOAD_URL = _GH_RAW + "gst_visitor_system_latest.py"

def _do_update_check(parent_win):
    """Background thread: check GitHub for newer version."""
    try:
        import urllib.request as _ur
        with _ur.urlopen(VERSION_URL, timeout=6) as r:
            raw = r.read().decode("utf-8").strip()
        lines     = raw.splitlines()
        new_ver   = lines[0].strip()
        changelog = "\n".join(lines[1:]).strip() if len(lines) > 1 else ""
        if new_ver == CURRENT_VERSION:
            return                          # already latest — do nothing
        # Newer version found — show popup on main thread
        parent_win.after(0, lambda: _show_update_popup(
            parent_win, new_ver, changelog))
    except Exception:
        pass                                # no internet or GitHub down — silent
...
def _show_update_popup(parent_win, new_ver, changelog):
    ...
    vin  = Frame(vrow, bg="#F8FAFF", padx=14, pady=10); vin.pack(fill=X)
    Label(vin, text=f"Current version : {CURRENT_VERSION}",
          font=("Segoe UI",9), bg="#F8FAFF", fg="#64748B").pack(anchor=W)
    Label(vin, text=f"New version     : {new_ver}",
          font=("Segoe UI",10,"bold"), bg="#F8FAFF", fg="#16A34A").pack(anchor=W)
    ...
    if changelog:
        ...
        Label(body, text=changelog,
              font=("Segoe UI",9),
              bg="#FFFFFF", fg="#475569",
              justify=LEFT, wraplength=380).pack(anchor=W, pady=(2,12))

    # Safety note
    Label(body,
          text="✅  Your data (Excel files, photos) will NOT be touched.",
          font=("Segoe UI",8,"bold"),
          bg="#FFFFFF", fg="#16A34A").pack(anchor=W, pady=(0,14))
    ...
    def _do_download():
        upd_btn.config(state=DISABLED, text="Downloading...")
        ...
        def _dl():
            try:
                ...
                if getattr(_sys, "frozen", False):
                    dest = os.path.abspath(_sys.executable)
                else:
                ...
                # Verify download is valid Python
                with open(tmp, "r", encoding="utf-8") as f:
                    content = f.read()
                if len(content) < 1000:
                    raise Exception("Downloaded file too small — may be corrupt")
                ...
                top.after(0, lambda: _fail(str(ex)))

        def _done():
            prog_var.set("✅  Update downloaded successfully!")
            top.update()
            messagebox.showinfo(
                "✅  Update Complete",
                f"GST VMS updated to {new_ver} successfully!\n\n"
        ...
        def _fail(err):
            prog_var.set(f"❌  Failed: {err}")
            upd_btn.config(state=NORMAL, text="⬆  Retry Update")
            skip_btn.config(state=NORMAL)
            messagebox.showerror(
                "Update Failed",
                f"Could not download update.\n\nReason: {err}\n\n"
                f"Check internet and try again.\n"
                f"Your current software still works normally.",
                parent=top)

        threading.Thread(target=_dl, daemon=True).start()

    upd_btn = Button(btn_row,
                     text="⬆  Update Now",
...
def check_for_update_async(parent_win):
    """Call this on startup — checks in background, zero UI freeze."""
    threading.Thread(
...
def show_login():
    ...
    # ── Auto-update check — runs in background, 3 seconds after login screen loads ──
...
def open_reception():
    ...
    def update_stats():
        def _do_read():
            try:
                ...
                df["Remaining"] = df["Remaining"].astype(str).str.strip()
                td    = df[df["Date"] == today]
        ...
        root.after(3000, update_stats)  # every 3 seconds
    update_stats()
    ...
    def show_returning_banner(name, phone, company, gst,
        ...
        loaded = False
        if WEBCAM_AVAILABLE and photo_path and os.path.isdir(str(photo_path)):
            try:
                imgs = [f for f in os.listdir(photo_path)
                        if f.lower().endswith((".jpg",".jpeg",".png"))]
                if imgs:
                    from PIL import Image as _PI, ImageTk as _IT
                    img = _PI.open(os.path.join(photo_path, imgs[0]))
    ...
    def _show_vip_generate_popup(name, phone, company, visits, photo_path=""):
        ...
        # Try load real photo
        photo_loaded = False
        if WEBCAM_AVAILABLE and photo_path and os.path.isdir(photo_path):
            try:
                imgs = [f for f in os.listdir(photo_path)
                        if f.lower().endswith((".jpg",".jpeg",".png"))]
                if imgs:
                    ...
                    img = PILImage.open(
                        os.path.join(photo_path, imgs[0]))
    ...
    def autofill_phone(event):
        ...
        try:
            ...
            if not old.empty:
                ...
                # Auto-fill SAFE fields only:
                # Name, Company, GST No, Purpose
                # DO NOT auto-fill: Officer, Division, Room, Block, Floor
                # Reason: officer may be different each visit — receptionist must ask
                ...
                # Purpose — auto-fill as suggestion (receptionist can change)
                if purpose and not entries["Purpose"].get().strip():
                    entries["Purpose"].set(purpose)

                # Officer, Division, Room, Block, Floor — LEFT BLANK intentionally
                # Receptionist must ask visitor each time
    ...
    def save():
        ...
        try:
            ...
            same_today = df_warn[
                (df_warn["Date"]==today_w) & (df_warn["Phone"]==phone)]
            ...
            if not still_inside.empty:
                ...
                if not messagebox.askyesno(
                    "⚠️  Same Visitor Already Inside Today",
                    f"Phone {phone} ({name}) already has\n"
                    f"active entry today:\n\n"
                    f"Group(s): {gids}\n\n"
                    f"This visitor may still be inside.\n"
                    f"Click YES only if this is a genuinely\n"
                    f"NEW visit (e.g. re-entered after exit).",
                    parent=root):
                    return
        except: pass
        try:
    ...
    def daily_report():
        ...
        df=pd.read_excel(VISITORS_FILE(),dtype=str)
        df["Date"]=df["Date"].astype(str).str.strip()
        rpt=df[df["Date"]==today]
        fn=f"GST_Report_{today.replace('-','_')}.xlsx"
        rpt.to_excel(fn,index=False)
        try:
